...
# 🔥 Flask & Core
from flask import Flask, render_template, redirect, url_for, flash, request, jsonify, send_file, make_response, abort
...
def register():
    ...
    if form.validate_on_submit():
        try:
            ...
            # ✅ Firebase save
            try:
                db_firestore.collection("users").document(user.registration_no).set({
            ...
            if 'UNIQUE constraint failed' in str(e) or 'unique' in str(e).lower():
                if 'registration_no' in str(e):
                    flash(_('That registration number is already taken.'), 'danger')
                elif 'email' in str(e):
                    flash(_('That email address is already registered.'), 'danger')
                else:
                    flash(_('Duplicate entry detected.'), 'danger')
            else:
                flash(_('Registration failed due to server error: %(error)s', error=str(e)), 'danger')
...
def report_lost():
    ...
    if form.validate_on_submit():
        ...
        db.session.commit()
          # Firebase save
        db_firestore.collection("lost_items").add({
            "item_name": item.item_name,
            "description": item.description,
            "date": str(item.lost_found_date),
            "location": item.location,
            "reported_by": current_user.registration_no
        })
